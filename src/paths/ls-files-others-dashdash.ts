// git ls-files --others --exclude-standard [-z] -- — same untracked,
// non-ignored listing as ls-files-others-exclude-standard, but with git's
// explicit empty pathspec separator.  Callers (t3code's staging sweep) emit
// the "--" defensively after flags; with no pathspecs after it the output is
// byte-identical to the separator-less form.  arc status --json -u all
// yields root-relative untracked paths that already respect .arcignore, so
// it is the direct source; git ls-files is cwd-scoped and cwd-relative, so
// the shim filters to the cwd subtree and rewrites.
...
export default definePath({
	name: "ls-files-others-dashdash",
	summary: "untracked non-ignored files with explicit empty pathspec separator",
	...
	fixtures: [
		{
			name: "NUL-terminated with empty pathspec separator",
		...
		{
			name: "newline-separated with separator, no -z",
			argv: ["ls-files", "--others", "--exclude-standard", "--"],
			arcReplies: {
				"status --json -u all": {
					stdout:
						'{"status":{"untracked":[{"status":"untracked","type":"file","path":"dir/new.txt"},{"status":"untracked","type":"file","path":"a.out"}]}}',
				},
			},
			want: { stdout: "a.out\ndir/new.txt\n", code: 0 },
		},
		{
			name: "no untracked files, -z with separator",
			argv: ["ls-files", "--others", "--exclude-standard", "-z", "--"],
			arcReplies: {
		...
		{
			name: "scoped to cwd subdirectory with separator",
			argv: ["ls-files", "--others", "--exclude-standard", "-z", "--"],
			cwd: "/arcadia/sub",
			arcReplies: {
				"status --json -u all": {
			...
			want: { stdout: "a.txt\0deep/b.txt\0", code: 0 },
		},
	],
})
