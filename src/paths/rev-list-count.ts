	...
	fixtures: [
		{
			...
			arcReplies: {
				"log --format={commit} -n 1000 arcadia/trunk..HEAD": { stdout: "aaa\nbbb\n" },
				"log --format={commit} -n 1000 HEAD..arcadia/trunk": { stdout: "ccc\n" },
			},
			want: { stdout: "2\t1\n", code: 0 },
		},
		{
			...
			arcReplies: { "log --format={commit} -n 1000 trunk..HEAD": { stdout: "" } },
			want: { stdout: "0\n", code: 0 },
		},
		{
			name: "origin alias normalized in range",
			argv: ["rev-list", "--count", "origin/trunk..HEAD"],
			arcReplies: { "log --format={commit} -n 1000 arcadia/trunk..HEAD": { stdout: "aaa\nbbb\n" } },
			want: { stdout: "2\n", code: 0 },
		},
	],
})
