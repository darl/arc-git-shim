// git diff --patch --minimal --ignore-all-space [<rev>] -- [<path>...]
...
// output shape is identical, only the chosen hunks may differ). --ignore-all-space
// maps 1:1 to arc's -w. The lone <rev> gets the same merge-base worktree lens as
// diff-prose (trunk drifts, so a literal `git diff trunk` would drown the caller's
// changes); paths after `--` pass through as arc diff path filters.
import { definePath, expandDiffRev, isExecResult, ok } from "../core"

export default definePath({
	name: "diff-patch-minimal-ignore-all-space",
	summary: "patch diff ignoring whitespace via arc diff --git -w",
	spec: "diff --patch --minimal --ignore-all-space <rev>? -- <paths...>?",
	...
	fixtures: [
		{
			...
			argv: ["diff", "--patch", "--minimal", "--ignore-all-space", "HEAD", "--"],
			arcReplies: {
		...
		},
		{
			...
			argv: ["diff", "--patch", "--minimal", "--ignore-all-space", "HEAD", "--", "foo.go"],
			arcReplies: {
				"merge-base HEAD HEAD": { stdout: "abc123def456\n" },
				"diff --git -w abc123def456 foo.go": {
					stdout:
						"diff --git a/foo.go b/foo.go\n--- a/foo.go\n+++ b/foo.go\n@@ -1 +1 @@\n-x\n+y\n",
				},
			},
		...
		},
		{
			...
			argv: ["diff", "--patch", "--minimal", "--ignore-all-space", "--"],
			arcReplies: {
				"diff --git -w": { stdout: "" },
			},
			want: { stdout: "", code: 0 },
